...
export function collectReachableFrames(_language, reachablesSlice) {
  ...
  for (const anode of reachableNodes) {
    ...
    if (isCryptoFlow && codeSnippets.length) {
      ...
      // findCryptoAlgos only recognises algorithms with an entry in the OID
      // dictionary. Some real, in-use algorithms have no registered OID at
      // all (e.g. Argon2id -- see CycloneDX/specification#756) and so can
      // never match there, even though atom's own crypto-algorithm tag on
      // the flow already correctly identifies them. Fall back to that tag
      // directly rather than silently dropping the evidence: a
      // cryptographic-asset entry without an `oid` is schema-valid
      // (cryptoProperties only requires `assetType`), so this is a real,
      // valid CBOM entry, not a workaround.
      if (!cryptoAlgos.length) {
        for (const fnode of anode.flows) {
          const ftagStr = fnode.tags || "";
          if (ftagStr.includes("crypto-algorithm") && fnode.name) {
            const ref = `crypto/algorithm/${fnode.name}`;
            if (!cryptoComponentsMap[ref]) {
              cryptoComponentsMap[ref] = {
                name: fnode.name,
                ref,
                description:
                  "Detected via a crypto-algorithm flow tag; no OID is registered for this algorithm",
              };
            }
            for (const cpurl of cpurls) {
              cryptoGeneratePurls[cpurl].add(ref);
            }
          }
        }
      }
